<template>
    <div class="bg-gray-100 min-h-screen p-6">
        ...
        <router-link :to="{ name: 'customer-service' }">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="h-6 w-6 cursor-pointer text-blue-500 hover:text-blue-600">
        ...
        />
        <EditTaskModal :isOpen="isEditModalOpen" :task="editingTask" @close-modal="isEditModalOpen = false" @task-updated="updateTask" />
  ...
  {{ task.name }}
  <div>
    <button @click="openEditModal(task)" class="bg-yellow-500 text-white rounded px-2 py-1 hover:bg-yellow-600 transition">Edytuj</button>
